...
STRING_LEN equ 48 ; Max string length
; basic io
extern printf
extern scanf
...
welcome_name: db "Welcome %s.", 10, 0
...
my_name: resb STRING_LEN
resistance: resb STRING_LEN
current: resb STRING_LEN
segment .text
...
computations:
    ...
    ; push qwords for stack alignment purposes
	push qword 0
	push qword 0
    ...
    mov rdi, my_name ; Load format into rdi
    ...
    mov byte [my_name + rax], 0
	
	; print welcome message
	mov rax, 0
	mov rdi, welcome_name
	mov rsi, my_name
	call printf
    ...
    mov rax, 0 ; Tell printf we have 0 floating point args
    mov rdi, format ; Load format into rdi
    mov rsi, prompt_resistance ; Load message into rsi
    call printf
	
	; get resistance from user, store in resistance
	mov rax, 0
	mov rdi, format
	mov rsi, resistance
	call scanf
    ...
    ; make call to C function ispositivefloat
	mov rax, 0
	mov rdi, resistance
	call ispositivefloat ; return value stored in rax
    ...
    mov rax, 0 ; Tell printf we have 0 floating point args
    mov rdi, format ; Load format into rdi
    mov rsi, prompt_current ; Load message into rsi
    call printf
	
	; get current from user, store in current
	mov rax, 0
	mov rdi, format
	mov rsi, current
	call scanf
